...
with st.spinner(
    f"Loading modules for ```article_page.py``` | Approximate loading time: ```{round(calculate_module_import_time(['objectify', 'article_analysis']), 4)}``` seconds"
):
    load_text = st.empty()
...
if submit_button:
    ...
    for i, cluster in enumerate(cluster_articles):
        if is_cluster_valid(cluster, keywords=keywords_bert, debug=True):
            cluster = objectify_and_summarize(cluster, light_checkbox)
    ...
    with st.container():
        ...
        for i, cluster in enumerate(valid_clusters):
            with st.expander(f"Cluster regarding **{unique_cluster_keywords[i].title()}**...", i == 0):
                ...
                for sentence in cluster["sentences"]:
                    stoggle_text += (
                        f"<li><b>[Source: {sentence.source}] [Date: {sentence.date}]:</b> {sentence.text}</li>"
                    )
                stoggle_text += "</ul>"
                ...
                st.markdown(
                    f"**Reliability:** <span style='color:{reliability_color}'>{reliability_text} "
                    f"(Score: {round(reliability, 3)})</span>",
                ...
                if not global_min_date or not global_max_date:
                    ...
                    if len(parsed_dates) == 0:
                        ...
                        if total_days < 1:
                            total_days = 1
                        timeline_div = (
                            "<div style='width: 100%; height: 90px; position: relative; margin-top: 20px;'>"
                            "<div style='position: absolute; top: 50%; transform: translateY(-50%); "
                            "width: 100%; height: 4px; background-color: #e0e0e0; border-radius: 2px;'></div>"
                        )
                        buffer = 5
                        offsets = []
                        ...
                        timeline_div += (
                            f"<div style='position: absolute; left: 0; top: 60%; font-size: 12px; white-space: nowrap;'>{first_label}</div>"
                            f"<div style='position: absolute; right: 0; top: 60%; font-size: 12px; text-align: right; white-space: nowrap;'>{last_label}</div>"
                        )
                        for off, lbl in offsets:
                            timeline_div += (
                                f"<div style='position: absolute; left: {off}%; top: 50%; transform: translate(-50%, -50%);'>"
                                f"<div style='width: 10px; height: 10px; background-color: black; border-radius: 50%;'></div>"
                                f"<div style='position: absolute; top: -35px; left: 50%; transform: translateX(-50%); "
                                f"font-size: 12px; white-space: nowrap;'>{lbl}</div>"
                                f"</div>"
                            )
                        if len(offsets) == 2:
                            off1, _ = offsets[0]
                            ...
                            if off1 != off2:
                                ...
                                timeline_div += (
                                    ...
                                    f"left: {left_off}%; width: {width}%; height: 2px; "
                                    f"border-bottom: 2px dotted black;'></div>"
                                )
                        if len(offsets) > 2:
                ...
                stoggle(
                    "Reliability Stats (Click to Expand)",
                    f"<b>Objectivity Score:</b> {round(obj_val*100, 2)}/100<br>"
                    f"<b>Sources:</b> {src_list_html}<br>"
                    f"<b>Timeline:</b><br>{timeline_div}"
                )
    progress_bar.empty()
